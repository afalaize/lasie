...
def eigen_decomposition(U):
    """
    Compute the eigen decompositon of matrix C[i,j] = U[x,c,i]*U[x,c,j]

    Parameters
    ----------

    U : numpy.array
        TimeSerie from wich to extract the data that form the correlation
        matrix C[i,j] = U[x,c,i]*U[x,c,j].
    ...
    eigen_vals : numpy.array
        ...
        complex type, unless the imaginary part is zero in which case it will
        be cast to a real type. When a is real the resulting eigenvalues will
        be real (0 imaginary part) or occur in conjugate pairs.

    eigen_vec : numpy.array
        The normalized (unit “length”) eigenvectors, such that the column
        eigen_vec[:,i] is the eigenvector corresponding to the eigenvalue
        eigen_vals[i].

    """

    # Form correlation matrix
    C = np.einsum('xci,xcj->ij', U, U)
...
def compute_kinetic_energy(data):
    """
    Return the kinetic energy associated with the velocity data with shape
    (nx, nt, nc).
    """
