...
def U_generator():
    for a in list_of_snapshots:
        yield a
U = misc.concatenate_over_2d_axis(U_generator())
...
def F_generator():
    for a in list_of_NL_snapshots:
        yield a
F = misc.concatenate_over_2d_axis(F_generator())
...
threshold = 1e-3
nmax = None
U_basis = pod.compute_basis(U_generator, threshold=threshold, nmax=nmax)
F_basis = pod.compute_basis(F_generator, threshold=threshold, nmax=nmax)
