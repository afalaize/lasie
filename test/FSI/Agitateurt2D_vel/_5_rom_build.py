...
from lasie_rom.rom import fsi_relaxed_velocity
...
if True:
    ...
    print('Build temp_a')
    temp_a = fsi_relaxed_velocity.temp_a(velocity_basis_hdf.basis[:])
    print('Build temp_b_rho')
    temp_b_rho = fsi_relaxed_velocity.temp_b_rho(velocity_basis_hdf.basis[:],
                                                  velocity_meanfluc_hdf.mean[:],
                                                  velocity_basis_hdf.basis_grad[:],
                                                  velocity_meanfluc_hdf.mean_grad[:])
    print('Build temp_b_nu')
    temp_b_nu = fsi_relaxed_velocity.temp_b_nu(velocity_basis_hdf.basis_grad[:])
    print('Build temp_c')
    temp_c = fsi_relaxed_velocity.temp_c(velocity_basis_hdf.basis[:], velocity_basis_hdf.basis_grad[:])
    print('Build temp_d')
    temp_d = fsi_relaxed_velocity.temp_d(velocity_basis_hdf.basis_grad[:])
    print('Build temp_f_rho')
    temp_f_rho = fsi_relaxed_velocity.temp_f_rho(velocity_basis_hdf.basis[:],
                                                  velocity_meanfluc_hdf.mean[:],
                                                  velocity_meanfluc_hdf.mean_grad[:])
    print('Build temp_b_nu')
    temp_f_nu = fsi_relaxed_velocity.temp_f_nu(velocity_basis_hdf.basis_grad[:],
                                                 velocity_meanfluc_hdf.mean_grad[:])
